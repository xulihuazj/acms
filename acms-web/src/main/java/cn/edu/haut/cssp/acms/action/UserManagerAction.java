...
public class UserManagerAction extends BaseAction{
	 ...
	 * @param:
	 */
	@RequestMapping(value = "/system/modifyPsd.do", method = RequestMethod.POST, produces = "applicaiton/json;charset=utf-8")
	@ResponseBody
	public Object modifyPsd(String oldPassword, String newPassword, String verifyPassword) {
		// 从session中获取当前用户
		TUser currUser = OperatorUtil.getOperator().getCurrUser();
		//旧、新、确认密码不为空
		if(StringUtils.isNotBlank(newPassword) && StringUtils.isNotBlank(newPassword)
				&& StringUtils.isNotBlank(verifyPassword)){
			// 旧密码是否相等、新密码与确认密码是否相等
			if(!PasswordUtils.encodePasswordSHA1(oldPassword).equals(currUser.getPassword())){
				return "旧密码不正确!";
			} else if(StringUtils.equals(newPassword, verifyPassword)) {
				return "新密码与确认密码不相等";
			} else {
				// 新密码加密
				currUser.setPassword(PasswordUtils.encodePasswordSHA1(newPassword));
	 ...
	 * @param:
	 */
	@RequestMapping("/system/checkUserName.do")
	@ResponseBody
